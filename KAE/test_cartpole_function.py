...
def test_cartpole_kae_function(kae, observable_dim, padded_dim, p, device, mode_number = -1, num_episodes = 20, save_imgs = False):
    ...
    with torch.no_grad():
        ...
        for _ in range(num_episodes):
            ...
            while not done:
                ...
                # else:
                #     logits = stt_decompose_mode(kae, z, observable_dim, mode_number, propagation = True)
    ...
    if save_imgs:
        ...
        ani = FuncAnimation(fig, update, frames=max(n_frames), interval=33)  # ~30 FPS
        ani.save("imgs/cartpole_all_episodes_subplots_with_time_mode_"+str(mode_number)+".gif", writer="pillow", fps=30)
        print("GIF saved as imgs/cartpole_all_episodes_subplots_with_time_mode_"+str(mode_number)+".gif")
